...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->bind(ShippingLabelGateway::class, function ($app): ShippingLabelGateway {
            $config = $app['config']->get('services.easypost', []);

            return new EasyPostShippingLabelGateway(
                http: $app->make(HttpFactory::class),
                apiKey: (string) ($config['api_key'] ?? ''),
                baseUrl: (string) ($config['base_url'] ?? 'https://api.easypost.com/v2'),
